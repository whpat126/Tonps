...
public class UserInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		/*
		 * 自动登录实现 1. 获取用户的session中的AuthenToken 存在：不执行任何操作 不存在： 2.
		 * 获取cookie中的用户ID，存在：获取该用户的详细信息，保存到session中 不存在： 3. 获取当前访问url 4.
		 * 获取web.xml中放行的地址 5. 如果访问的url不是放行的地址，跳转到登录页面
		 */
			...
			if (cookies != null && cookies.length > 0) {
				...
				for (Cookie cookie : cookies) {
					...
					if ("user".equals(cookie.getName())) {
						...
						// Users user2 =
						// baseUserService.findById(Long.parseLong(userId));
						usernameAndPwd = cookie.getValue();
						String[] str = usernameAndPwd.split("@@@@");
						System.out.println("loginInterceptor的for循环中" + str[0]
								+ " :  " + str[1]);
