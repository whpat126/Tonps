	var rectangleBorderWidth = 2;	// Used to set correct size of the rectangle
								// with red dashed border
	var useRectangle = false;	
	var autoScrollSpeed = 4;	// Autoscroll speed - Higher = faster
	
	/*
	 * The saveData function creates a string containing the ids of your
	 * dragable elements.
	 * 
	 ...
	 * 
	 * i.e. a semi colon separated list. The id is something you put in as "id"
	 * attribute of your dragable elements.
	 * 
	 */
	...
	{
		var saveString = "";
		for(var i=0;i<dragableObjectArray.length;i++){
			if(saveString.length>0)
				saveString = saveString + ';';
			ref = dragableObjectArray[i];
		 ...
		 * 
		 * On the server explode the values by use of server side script. Then
		 * update your database with the new item order
		 * 
		 */
	}
	...
	var dragDropTimer = -1;	// -1 = no drag, 0-9 = initialization in progress,
							// 10 = dragging
	...
	{
		var height = dragObject.offsetHeight; // 得到对象的高度
		var width = dragObject.offsetWidth; // 得到对象的宽度
		var indexCurrentDragObject=-5; // 拖动对象的索引
		for(var no=0;no<dragableObjectArray.length;no++){			
			ref = dragableObjectArray[no];			
			if(ref['obj']==dragObject)
				indexCurrentDragObject=no;
			if(no<dragableObjectArray.length-1 && dragableObjectArray[no+1]['obj']==dragObject)
				indexCurrentDragObject=no+1;
			if(ref['obj']==dragObject && useRectangle)
				continue;	
			...
			if(x > (ref['left'] + (ref['width']/2)) && y>ref['top'] && x<(ref['left'] + ref['width']) && y<(ref['top'] + ref['height'])){
				if(no<dragableObjectArray.length-1){
					...
					if(dragableObjectArray[no]['obj']!=dragObject){
						...
						if(!useRectangle)
							return 'self';
						if(no<dragableObjectArray.length-2)
							return Array(dragableObjectArray[no+2],no+2);
					}
				}else{
					if(dragableObjectArray[dragableObjectArray.length-1]['obj']!=dragObject)
						return 'append';	
				}
			}
			if(no<dragableObjectArray.length-1){
		...
		if(x>ref['left'] && y>(ref['top'] + ref['height']))
			return 'append';				
		return false;	
	}
	...
	function initDrag(e)
	{
		if(document.all) // document.all判断浏览器是否是IE。
			e = event; 
		mouse_x = e.clientX;
		mouse_y = e.clientY;
		if(!documentScrollHeight)
			documentScrollHeight = document.documentElement.scrollHeight + 100; // 页面的滚动高度
		...
		if(this.nextSibling){ // nextSibling 指被选节点的下一个节点
			dragObjectNextObj = this.nextSibling;
			if(!dragObjectNextObj.tagName)
				dragObjectNextObj = dragObjectNextObj.nextSibling;
		}
		initDragTimer();
		return false;
	}
	...
	function initDragTimer(){
		...
		if(dragDropTimer==10){
			if(useRectangle){
				...
				var newObject = dragObject.cloneNode(true); // 克隆一个节点
	...
	function initdragableElements()
	{
		...
		for(var no=0;no<subDivs.length;no++){
			...
			if(opera)attr = subDivs[no].dragableBox;
			if(attr=='true'){
				subDivs[no].style.cursor='pointer';	
